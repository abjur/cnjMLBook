...
maiores_litigantes_telecom %>%
  head(4) %>%
  knitr::knit_print(caption = "4 maiores litigantes do seor das telecomunicações do estado da Bahia.")
```
...
d <- readRDS('data-raw2/d_senacon.rds')

library(tidyverse)
```

A secretaria Nacional de Atendimento ao Consumidor criou o site consumidor.gov em 2013, já recebeu mais de `r "679015"` reclamações nos seus quase 4 anos de duração. Nessa página, os consumidores podem entrar em contato direto com as prestadoras de serviço listadas na plataforma para resolver conflitos consumeristas, sem precisar recorrer às vias legais. Neste capítulo, vamos mapear as principais queixas cadastradas e mensurar o sucesso da plataforma à luz dos resultados obtidos nos tribunais.
...
Essa vantagem se mantém inclusive se compararmos os processos pelos diferentes assuntos. Tomando apenas as instituições financeiras e empresas de telefonia e assuntos com mais do que 1000 reclamações, a média de efetividade de todos os assuntos em empresas de telecomunicações é `r "77,1%"`, enquanto o mesmo valor para instituições bancárias é `r "51%"`.
...
uniao_tjrs_senacon %>% 
  ...
  set_names("Empresa", "Número de processos", "Proporção de casos resolvidos", "Percentual máximo\n de litigantes que consultaram\no consumidor.gov", "Redução no número\nde processos (pior cenário)",
    ...
    knitr::knit_print(caption = "Proporção de casos resolvidos e percentual máximo de litigantes que consultarma o consumidor.gov para 6 empresas, com estimativas de ganho no cenário contrafactual.")
