...
```{r f4, fig.cap = "Volume processual ao longo do tempo no Rio Grande do Sul contra companhias aéreas. Fonte: Tribunal de Justiça de Rio Grande do Sul."}
totais <- d_tjrs_2 %>%
...
assuntos_bancos %>%
  head(10) %>%
  knitr::knit_print(caption = "10 causas de pedir consumeristas mais frequentes conta instituições financeiras no estado do Rio de Janeiro. Fonte: Selo Justiça em Números.")
```
...
assuntos_telecom %>%
  head(10) %>%
  knitr::knit_print(caption = "10 assuntos consumeristas mais frequentes contra empresas de telecomunicação no estado do Rio de Janeiro. Fonte: Selo Justiça em Números.")
```
...
assuntos_energia %>%
  head(10) %>%
  knitr::knit_print(caption = "10 assuntos consumeristas mais frequentes contra empresas de concessionárias de serviçoes básicos no estado do Rio de Janeiro. Fonte: Selo Justiça em Números.")
```
...
```{r, fig.cap = "Número de processos consumeristas distribídos no estado de São Paulo ao longo do tempo separado por setor econômico da demandada. Fonte: Tribunal de Justiça de São Paulo."}
d_tjsp_2 %>%
  count(mes, area) %>%
...
maiores_telecomunicacoes %>%
  head(10) %>%
  knitr::knit_print(caption = "10 maiores litigantes consumeristas do setor das telecomunicações no estado do Mato Grosso. Fonte: Selo Justiça em Números.")
```
...
```{r, fig.cap = "Distribuição dos assuntos processuais nas 4 companhias de seguros maiores litigantes consumeristas do estado do Mato Grosso. Fonte: Selo Justiça em Números."}
d_tjmt_2 %>%
...
```{r, fig.cap = "Volume processual dos processos consumeristas nos quatro maiores setores econômicos. Fonte: Elaboração própria."}
d_tjba_2 %>%
...
```{r}
head(assuntos_bancos, 10) %>%
  knitr::knit_print(caption = "10 assuntos mais comuns. Fonte: Elaboração própria.")
```
...
```{r}
assuntos_seguros %>%
  knitr::knit_print(caption = "5 causas de pedir consumeristas mais comuns no estado da Bahia. Fonte: Elaboração própria.")
```
...
maiores_litigantes_telecom %>%
  head(4) %>%
  knitr::knit_print(caption = "4 maiores litigantes do seor das telecomunicações do estado da Bahia. Fonte: Elaboração própria.")
```
...
d %>% 
  ...
  knitr::knit_print(caption = "Reclamações por 100mil reais do PIB separadas por unidade da federação. Fonte: SENACON.")
```
...
```{r fig.cap="Número de reclamações por mês em cada UF. Fonte: SENACON."}
d %>% 
  mutate(mes = lubridate::floor_date(data_finalizacao, "months")) %>% 
  group_by(uf, mes) %>% 
  count() %>% 
...
```{r, fig.cap = "Distribuição da proporção de casos resolvidos, separados por setor econômico. Fonte: SENACON."}
d %>% 
  filter(avaliacao_reclamacao != "Não Avaliada") %>%
...
uniao_tjrs_senacon %>% 
  ...
  set_names("Empresa", "Número de processos", "Proporção de casos resolvidos", "Percentual máximo\n de litigantes que consultaram\no consumidor.gov", "Redução no número\nde processos (pior cenário)",
    ...
    knitr::knit_print(caption = "Proporção de casos resolvidos e percentual máximo de litigantes que consultarma o consumidor.gov para 6 empresas, com estimativas de ganho no cenário contrafactual. Fonte: SENACON e Tribunal de Justiça do Rio Grande do Sul.")
