...
O roteiro limitou-se às cifras mencionadas acima (5 setores, 10 assuntos processuais e 30 litigantes) apenas para tornar a exposição mais clara. Conforme verificado a seguir, esses números foram capazes de descrever o comportamento de grande quantidade dos processos. Mesmo assim, para os casos não comentados nesse capítulo, o apêndice contém versões menos resumidas das tabelas.
  ...
  Em alguns tribunais, os mecanismos de seleção impossibilitam análises temporais. Isso acontece porque os dados utilizados na premiação do Selo Justiça em Números não representam adequadamente os processos distribuídos em um ano. Os dados obtidos na ocasião desse prêmio representam adequadamente os processos que receberam carga e aqueles processos que estavam em aberto no momento da coleta. Por conta disso, fizemos análises temporais apenas conduzimos estudos prospectivos, nos estados de São Paulo, Rio Grande do Sul, Amazonas e Bahia.
...
maiores_telecomunicacoes %>%
  head(7) %>%
  knitr::knit_print(caption = "7 maiores litigantes consumeristas do setor das telecomunicações no estado do Mato Grosso.")
```
...
d %>% 
  ...
  select(id, n) %>% 
  set_names(c("UF","Reclamações por 100mil reais do PIB")) %>% 
  knitr::knit_print(caption = "Reclamações por 100mil reais do PIB separadas por unidade da federação.")
```
...
uniao_tjrs_senacon %>% 
  ...
  set_names("Empresa", "Número de processos", "Proporção de casos resolvidos", "% que consultou o site", "Redução % (pior cenário)",
            "Redução % (cenário médio)",
            "Redução % (melhor cenário)") %>%
