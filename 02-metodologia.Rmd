...
evitar novos casos. Nesse capítulo, descreveremos a
metodologia utilizada para solucionar os problemas de cada parte.

Vale ressaltar que essa pesquisa tem base filosófica pragmática (\citep{hanson2005mixed}). Isso significa que o foco principal são resultados aplicáveis
na sociedade. O rigor metodológico reside principalmente na parte
exploratória, especificamente nos métodos estatísticos de análise e
inferência.

A pesquisa também busca atender o \emph{princípio da reprodutibilidade} (\citep{gelman2014bayesian}). 
Com isso será possível utilizar os avanços
descritos para aplicar em outros trabalhos, garantindo um efeito
maior do que apenas a documentação de resultados.

A metodologia do estudo possui um desenho misto (\citep{hanson2005mixed}, p.~116).
Especificamente, utilizamos a técnica de métodos convergentes em
paralelo (\emph{convergent parallel methods design}). 
Nessa abordagem, coletamos e analisamos dados
qualitativos e quantitativos em paralelo, confrontando os resultados
para gerar interpretações e conclusões. A Figura \ref{fig:desenho} mostra
esquematicamente o desenho utilizado.

\begin{figure}
\begin{centering}
\includegraphics{creswell}
\caption{Método misto em paralelo convergente (adaptado de \citep{creswell2013research}, p. 270).}
\label{fig:desenho}
\end{centering}
\end{figure}

A parte exploratória utiliza métodos quantitativos baseados em dados
observacionais para elucidar o fenômeno de interesse (\citep{hanson2005mixed}, p.~18). Nesse caso,
iniciamos com a obtenção de dados para posterior análise e interpretação
de resultados. A Figura \ref{fig:quanti} mostra essa parte de forma esquemática.

\begin{figure}
\begin{centering}
\includegraphics[width=\linewidth]{quanti}
\caption{Metodologia quantitativa. Adaptado de \citep{wickham2016r}.}
\label{fig:quanti}
\end{centering}
\end{figure}

Já a parte propositiva utiliza tanto métodos quantitativos
...
A convergência das duas partes da pesquisa se deu na forma usual
(\citep{creswell2013research}, p.118). Os resultados da parte exploratória são utilizados como
...
A seguir, descrevemos a metodologia em maior detalhe. A Seção \ref{parte-exploratoria}
contempla os métodos de obtenção e análise dos dados. A Seção \ref{parte-propositiva}
descreve as atividades realizadas da parte propositiva.

## Parte exploratória} {#parte-exploratoria}

A pesquisa dos maiores litigantes gerou produtos úteis
para o estudo empírico em direito. Buscamos organizar esses avanços em
pequenos artigos. Tais artigos foram contemplados na
metodologia ou em apêndices do relatório.
...
situação atual; iii) a execução; iv) a validação dos resultados.

Os resultados dessas investigações foram resumidos abaixo.

\begin{enumerate}
\def\labelenumi{\arabic{enumi}.}
\item
  Queremos propor soluções para melhorar índices de litigiosidade.
\item
  \begin{enumerate}
  \def\labelenumii{\alph{enumii})}
  
  \item
    Identificamos que processos cíveis do primeiro grau na Justiça
    Estadual não fiscais e as execuções fiscais são os maiores
    responsáveis pelo volume processual do país; b) não somos capazes de
...
\item
  ...
  \item
    Recorte da pesquisa; b) extração de dados dos tribunais; c) criação
    de método para contagem de processos consumeristas; d) criação de
    técnicas para consolidação das partes dos processos.
  \end{enumerate}
\item
...
Cada item ganhou uma subseção especial de detalhamento, que completam a
parte exploratória do nosso trabalho.

### Direção da pesquisa} {#direcao-da-pesquisa}

O tema de interesse da pesquisa é a \textbf{administração judiciária}.
Especificamente, queremos propor soluções estratégicas para
...
Não temos interesse em estudar a quantidade de casos novos por
magistrado ou servidor. Essas quantidades serão fixadas para simplificar
a análise. Também por simplicidade, não temos interesse em monitorar ou
alterar recorribilidade interna.
...
As ações estratégicas serão focadas nos maiores litigantes em casos
consumeristas. Espera-se que soluções para essa subpopulação surtam
efeitos significativos e perenes na Justiça Brasiliera. Essa afirmação
foi feita inicialmente sob a hipótese de que casos consumeristas
representam uma quantidade relevante do total de casos. Essa hipótese
foi validada após extração e levantamento dos dados.

### Resumo da situação atual}{#resumo-da-situacao-atual}

Começamos o estudo um levantamento baseado no Relatório Justiça em
Números (RJN), ano base 2016, que utiliza dados enviados pelos tribunais
referentes ao ano de 2015. Essa análise tem como objetivo dar uma visão
geral do que estamos enfrentando e quais são os passos.

O presente estudo preliminar considerou somente as métricas de
litigiosidade do RJN: os casos novos, os casos pendentes e a quantidade
de processos baixados. Os casos pendentes refletem a situação atual do
judiciário; os casos novos relacionam-se com a demanda futura e os
processos baixados referem-se à eficácia.
...
faz sentido considerar a Justiça Estadual como foco.

\begin{table}

\caption{\label{tab:jntot}Casos novos e casos pendentes por Justiça.}
\centering
\begin{tabular}[t]{lrr}
\hline
...
\caption{\label{tab:inst}Casos novos e casos pendentes por grau. O TJAM não enviou dados da segunda instância no ano de 2015. Nesse caso, consideramos esse valor como zero.}
\centering
\begin{tabular}[t]{lrr}
\hline
...
O terceiro filtro aplicado relaciona-se com as áreas processuais cível
ou criminal. A Tabela \ref{tab:crim} mostra a quantidade de casos novos
e pendentes no primeiro grau e nos Juizados especiais, comparando as
duas áreas. A área cível é responsável por 88\% dos casos pendentes.
Mesmo com todos os filtros aplicados, ainda estamos com uma subpopulação
de mais de 50 milhões de casos pendentes.

\begin{table}

\caption{\label{tab:crim}Casos novos e casos pendentes por área, dentro do primeiro grau ou Juizado Especial. TJPI não enviou alguns dados de Juizados Especiais no ano de 2015. Nesse caso, consideramos esse valor como zero.}
\centering
\begin{tabular}[t]{lrr}
\hline
...
O próximo filtro é baseado no problema da execução fiscal. Para fins de
administração dos tribunais brasileiros, existem dois tipos de processos
principais: execução fiscal e outros. Esses casos são muito relevantes
em volume processual, especialmente na Justiça Estadual.

A Tabela \ref{tab:fiscal} mostra a quantidade de casos novos e pendentes
cíveis no primeiro grau e nos Juizados especiais, comparando execuções
fiscais com todo o resto. Metade dos casos pendentes são de execução
fiscal, enquanto que somente 16\% dos casos novos são de execução
fiscal. Isso indica que, além de volumosos, os casos de execução fiscal
apresentam alta taxa de congestionamento. Uma análise mais aprofundada
do tema foi descrita no Relatório Justiça em Números.

Nesse momento foi feita uma importante decisão de escopo: estudar apenas
os casos cíveis não relacionados à execução fiscal. Agora, nossa
população de casos pendentes tem aproximadamente 25 milhões de
processos.

\begin{table}

\caption{\label{tab:fiscal}Casos novos e casos pendentes cíveis por tipo de processo, dentro do primeiro grau ou Juizado Especial. TJPI não enviou alguns dados de Juizados Especiais no ano de 2015. Nesse caso, consideramos esse valor como zero.}
\centering
\begin{tabular}[t]{lrr}
\hline
...
Execução judicial ou extrajud. não fiscal & 1.952.634 (14,3\%) & 5.551.945 (11,1\%)\\
\hline
...
\item
  Como evitar a entrada de casos desnecessários dentro desses quase 10
  milhões anuais?
\end{enumerate}

Para isso, vamos realizar novos filtros e considerar o conteúdo dos
processos, não somente o volume. Nosso objetivo é identificar os tipos
processuais mais frequentes.

### Execução dos trabalhos {#execucao-dos-trabalhos}

O próximo filtro aplicado considera algumas UFs
de interesse. Esse filtro não está relacionado com a população-alvo da
pesquisa, pois as conclusões e propostas devem ser aplicáveis a todos os
tribunais estaduais. No entanto, um corte no escopo foi feito para
garantir que a pesquisa seria exequível.

Outro detalhe importante é que esse corte em especial não foi pensado
unicamente pelo volume processual. Nesse caso, consideramos também a
necessidade de incluir pelo menos uma UF por região brasilera, além de
...
de cada porte; e iii) a possibilidade de contemplar estados responsáveis
por considerável proporção dos litígios em cada região.
...
liberdade para escolha de estados. No entanto, é de extrema importância
...
poderia ser ruim considerar apenas uma Unidade Federativa nesta região.
Por isso, foi necessário considerar o TJDFT e dois tribunais do Sudeste,
além de um Tribunal para cada região do Brasil.

Durante a primeira fase do projeto, produzimos um ofício com pedidos de
listagens de processos, contendo diversas variáveis e especificações. Os
resultados das extrações foram descritos em Subseções próprias.

No final, acabamos considerando o TJMT no lugar do TJMS por conta da disponibilidade dos dados. Dessa forma, a pesquisa ficou apenas com um tribunal de pequeno porte, três de médio porte e três de grande porte.

A Tabela \ref{tab:tabuf} mostra a quantidade de casos novos e pendentes
cíveis de conhecimento no primeiro grau para cada Tribunal escolhido. Já
a Figura \ref{fig:tempo} mostra o volume de casos novos e pendentes ao
longo dos anos. Observa-se que as quantidades não são estáveis, mesmo
considerando Tribunais de grande porte.

\begin{table}

\caption{\label{tab:tabuf}Casos novos e casos pendentes cíveis por tribunal. Os porcentuais foram calculados em relação ao total de casos considerando todos os Tribunais.}
\centering
...
\begin{figure}
\centering
\includegraphics[width=\linewidth]{08-metodologia_files/figure-latex/tempo-1.pdf}
\caption{\label{fig:tempo}Volume de casos novos e pendentes ao longo do
tempo nos tribunais de interesse.}
\end{figure}

No Relatório Justiça em Números, é possível observar que assuntos
envolvendo o direito do consumidor aparecem dentre os assuntos mais
frequentes, especialmente nas turmas recursais e juizados especiais. No
entanto, o próprio relatório enfatiza que a análise não é completamente
confiável. Nosso intuito é avançar nesse sentido, produzindo estimativas
mais confiáveis.

Cada tribunal exigiu metodologias distinta para separação dos casos consumeristas. A Subseção \ref{extracao} trata desse tema.
...
arquivos de acompanhamento processual e autos do processo, iii)
...
\begin{centering}
\label{fig:arrumacao}
\includegraphics[width=\linewidth]{classificacao}
...
\paragraph{Contatos com os tribunais}\label{contatos-tribunais}
...
\paragraph{Resumo}\label{resumo-tribunais}
...
\paragraph{Assuntos - consumeristas}\label{assuntos}
...
\paragraph{Assuntos - cifra oculta}\label{assuntos}
...
\begin{figure}
\includegraphics[width=0.5\linewidth]{tpu} \caption{Árvore de assuntos relacionada ao Direito do Consumidor (parcial). Último acesso em 27/06/2017.}\label{fig:tpu}
\end{figure}
...
\begin{figure}
\centering
\includegraphics[width=\linewidth]{08-metodologia_files/figure-latex/rb-1.pdf}
...
\begin{figure}
\centering
\includegraphics[width=\linewidth]{08-metodologia_files/figure-latex/rb2-1.pdf}
...
\paragraph{Empresas - filtro}\label{empresas1}
...
\paragraph{Empresas - limpeza incorporações}\label{empresas2}
...
\begin{figure}
\centering
\includegraphics[width=\linewidth]{08-metodologia_files/figure-latex/comparacao-1.pdf}
...
\begin{figure}
\centering
\includegraphics[width=\linewidth]{08-metodologia_files/figure-latex/iadn2-1.pdf}
