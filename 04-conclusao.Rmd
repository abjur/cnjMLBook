...
de aprimorar a administração judiciária, posto que tais sugestões não se limitam apenas à justiça consumerista.
...
mudança legislativa. Entendemos que tais recomendações são capazes de
...
levar o problema a juízo. Seguindo esse trânsito, garantimos que houve uma
tentativa de comunicação entre as partes através do consumidor.gov.br, sem
gerar demandas adicionais ao judiciário e ao consumidor.

A Figura \@ref(fig:senacon) mostra o fluxo simplificado recomendado. Dado um
...
adicionado um campo para inserir o código identificador de reclamações
prévias do mesmo tema no consumidor.gov.br. Se o autor da ação deixar
esse campo em branco, uma reclamação é gerada paralelamente a fim de criar uma tentativa de conciliação extrajudicial. Se o conflito for resolvido pelo canal do consumidor.gov.br, o processo é encerrado. Em caso de o conflito
não ser resolvido em um dado prazo, a ação é automaticamente distribuída
e o processo corre normalmente.

```{r senacon, fig.cap = 'Fluxo de integração genérico.'}
#knitr::include_graphics('imgs.senacon.pdf')
```
...
1. Não atrasa o processo, visto que i) o prazo de atendimento no   consumidor.gov.br é curto e ii) os diálogos realizados através da plataforma, mesmo os travados ou ausentes, auxiliam na tomada de decisão no processo judicial, se este existir.

1. Evita a entrada de ações desnecessárias, ou seja, que podem ser resolvidas
  extrajudicialmente.

1. Desincentiva a entrada de litigantes e advogados oportunistas, uma vez
  que o consumidor.gov.br terá registro das discussões travadas entre
  reclamante e empresa.

1. Auxilia na documentação das reclamações, em razão do
  consumidor.gov.br possuir modelos de dados mais adequados para
  registrar informações sobre as queixas do que os sistemas dos
  tribunais.

1. Não afeta negativamente o acesso à justiça, já que a solução se aplica apenas ao peticionamento eletrônico.
...
pessoa busca uma composição amigável via SAC ou ferramentas como o
Reclame Aqui, ela deve repetir o processo no consumidor.gov.br para entrar com a ação. Ainda assim, considerando a velocidade de atendimento do
...
relevante, uma vez que afeta todos os peticionamentos eletrônicos
quando finalizada. Essa também é uma forma adequada de aumentar a
eficiência do judiciário, dado que a ferramenta pode ser utilizada somente quando
necessário.
...
é a criação de *dashboards* interativos e ferramentas de alerta.
...
judiciais é utilizando os chamados *assuntos processuais*. Os
...
CNJ^[http://www.cnj.jus.br/images/stories/docs_cnj/resolucao/rescnj_46.pdf.
...
assuntos e movimentações dos processos. As TPUs foram implantadas em
...
um caso que discute sobre "Análise de Crédito" classificado como "Responsabilidade do Fornecedor", ou ainda "Direito do Consumidor".

A possibilidade de subestimação do volume real de processos de um certo
tipo configura o que chamamos de **cifra oculta**. Dado um assunto
...
*differences in differences* [@bertrand2004much], que usualmente resultam em estimativas confiáveis dos efeitos das medidas.
...
*white lists*^[Uma *white list* é uma lista de nomes
...
técnicas como regressão LASSO [@friedman2001elements] em *sacolas de palavras*^[*Bag of words* ou sacola de palavras é uma 
...
A solução final foi implementada num pacote escrito em R chamado `tidyML`.^[https://github.com/abjur/tidyML. No
...
Para a realização desse projeto, foi necessário pesquisar o histórico das
...
[@Fossaluza2015] discute o tema da alocação intencional na condução
...
possível, pois o efeito estaria confundido [@pearl2009causality].
...
para obter dados de *tweets*, publicações no Facebook, entre
...
do fluxo da ciência de dados ([@wickham2016r]).
...
colunas, numa planilha padronizada. No entanto, tudo o que consegue encontrar
...
ofícios. Por isso, construímos *web scrapers*, que são robôs que
...
Contudo, são raros os profissionais que dominam esse conhecimento. A
ABJ disponibiliza abertamente todo seu aparato técnico ^[Disponíveis
  nos links:https://github.com/abjur (códigos genéricos da ABJ),
  https://github.com/courtsbr (web scrapers), e
  https://github.com/decryptr (ferramentas para quebrar CAPTCHAs).],
mas as ferramentas não são capazes de resolver qualquer problema. E os
sistemas dos Tribunais não cooperam: colocam impedimentos técnicos de
acesso e captchas (textos que vêm em imagem com o intuito de verificar se
o acesso não é de um robô), dificultando muito o trabalho.

Há muitos exemplos em que simplesmente não é possível obter as
informações que necessitamos. Isto é irônico ao passo de que (quase) todos os
processos são públicos. Em muitos casos a única forma de driblar o
problema se dá utilizando a Lei de Acesso à Informação, que, apesar de ser ótima, nos torna agentes passivos nas pesquisas e congestiona os setores
administrativos dos Tribunais.

Mais interessante do que rastrear e estruturar dados, é resolver o
problema na sua raiz. A solução envolve modificar os sites dos
Tribunais, permitindo extrações de dados e disponibilizar APIs que
permitam o pesquisadores de buscar as informações públicas de maneira
segura e organizada.
...
disso, essas soluções não causariam impactos negativos nos sistemas. Pelo contrário: ao permitir que os dados sejam baixados dessa
...
É próprio e esperado da pesquisa apresentada que o
...
processos ([@abj2015]). Uma alternativa para
solucionar esse problema é realizar uma amostra aleatória de varas e
...
indústria. Por um lado, isso poderia ser explicado por magistrados com tendência a favorecer o autor da ação, ou seja, que arbitram valores de indenização altos o
...
teve como porposta alertar a sociedade de que a litigiosidade é
...
```{r litig, fig.cap = 'Gráfico de dispersão do IDH-Médio estadual e quantidade de casos novos por cem mil habitantes.'}
#knitr::include_graphics('imgs.litig-1.pdf')
```
...
À vista disso, seria interessante realizar uma investigação mais aprofundada
