...
```{r litig, echo=FALSE, warning=FALSE, message=FALSE, fig.cap='Gráfico de dispersão do IDH-Médio estadual e quantidade de casos novos por cem mil habitantes.', fig.height=4, fig.width=7}
library(tidyverse)
...
pnud_uf <- pnud_uf %>% 
  ...
  select(-ano, -uf)
corr <- rcnj::d_jn$dados$`JN_18-Out-2016.csv` %>% 
  ...
  select(idhm, ch) %>% 
  with(cor(idhm, ch))
rcnj::d_jn$dados$`JN_18-Out-2016.csv` %>% 
  filter(ano == 2015, justica == 'Estadual', sigla != 'TJ') %>% 
  inner_join(pnud_uf, c('uf_sede' = 'uf_sigla')) %>% 
  mutate_at(vars(ch), parse_number, locale = locale(decimal_mark = ',')) %>% 
